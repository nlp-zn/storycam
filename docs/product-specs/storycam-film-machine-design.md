...
4. canonical 创作闭环必须保留：输入想法 -> 生成剧本、人物资产、场景资产 -> 用户确认 -> 生成分镜脚本和 1-3 个核心分镜组 -> 可选扩展 -> 每个确认的核心分镜组生成一个 Seedance 2.0 视频片段 -> 用户确认片段 -> 系统建议拼接 -> 最终作品。
...
- 用户可见语言、隐私边界、provider-send confirmation、核心分镜组数量规则。
...
| `script` | 是 | 一段可读短剧本，用来让用户判断 StoryCam 是否理解了自己的想法。 |
| `character_assets[]` | 是 | 1-3 张人物卡，包含角色、关系、稳定视觉描述、情绪、服装、道具和一致性说明。 |
| `scene_assets[]` | 是 | 1-3 张场景卡，包含地点、时间、光线、氛围、关键物件和空间逻辑。 |
| `storyboard_script` | 部分可见 | 轻量拍摄计划，用普通用户语言表达；专业分镜表留在内部。 |
| `core_storyboard_groups[]` | 是 | 1-3 个确认后的电影感核心时刻。每个组都是一个潜在视频片段。 |
| `expanded_storyboard_cards[]` | 是 | 核心组内的 3-8 张指导卡：进入、动作、反应、氛围、转场、情绪点或续写。 |
...
私人想法
  ...
  -> 每个确认组生成一个 Seedance 2.0 视频片段
  -> 用户确认片段
  -> 系统给出拼接建议
  -> 最终 10-15 秒作品
```

MVP 可以从一个分镜组、一个视频片段开始，但设计上必须保留 1-3 组结构，避免后续扩展时重写产品模型。
...
- `storyboard_script`
- 1-3 个 `core_storyboard_groups[]`

核心分镜组数量规则：

- 核心分镜组数量由计划生成的视频长度自动判断。
- 8-12 秒：默认 1 组。
- 10-14 秒：默认 2 组。
- 12-15 秒：默认 3 组。
- 系统可以根据故事密度下调数量；不要为了凑满三组拆散一个完整情绪时刻。

需要的 AI 能力：

- 从短剧本中抽取核心情绪转折。
- 把故事拆成 1-3 个可生成 clip 的镜头组。
- 使用内部 Shanyin-style 方法做节奏和动作-反应判断，但只输出用户能理解的核心方案。

模型选择边界：

- 具体文本模型和 prompt 编排在 PLAN 中确定。
- 是否生成核心分镜代表图、调用哪个图像模型，在 PLAN 中确定。
...
- 已确认的故事世界
- 已有扩展卡
- 用户操作：生成更多、换角度、加强情绪、删除

产出：

- 3-8 个 `expanded_storyboard_cards[]`

需要的 AI 能力：

- 根据父核心组生成进入、动作、反应、氛围、转场、情绪点等卡片。
- 保持人物、场景、情绪连续。
- 把 Shanyin 的镜头组逻辑转译为普通用户能看懂的“这一段会这样拍”。

模型选择边界：

- 扩展卡可以先用文本结构生成；是否生成每张卡的图片，以及调用哪个图像模型，在 PLAN 中确定。
- 扩展卡默认不触发视频生成。
...
- 2-3 个 clips 时，按建议顺序、时长、转场和字幕节奏合成。
...
5. 根据计划视频长度自动判断生成 1 组、2 组或 3 组核心分镜组。
6. 可选把选中组展开成 3 张指导卡。
...
Phase 1 不把第一个 clip 直接当作最终完成状态跳过合成；即使只有一个 clip，也需要生成 `final_work` artifact。界面和数据结构应保留“继续拍第二段/第三段”的路径。
...
- 系统不为了凑数强行生成 3 组；如果 1 组更干净，就生成 1 组。
...
- 默认扩展 3 张卡。
- 最多可见 8 张卡。
- 未生成 slot 可以展示 waiting/loading 状态。
...
UI 不应把它解释成“模型调用次数”，而应解释成“一个时刻”“两个时刻”“三个时刻”。

分组数量由用户计划生成的视频长度自动判断；用户不需要手动选择“调用几次模型”。
...
- MVP 中 Seedance 2.0 调用数等于确认的核心分镜组数量，通常是 1-3。
- 扩展分镜卡不会自动创建额外视频调用。
...
- 给定 `我想把暗恋拍成韩剧雨夜`，StoryCam 产出可读剧本、人物资产、场景资产、按计划视频长度自动判断的 1-3 个核心分镜组、可选扩展卡、clip prompt packet、根据 mode 生成真实或 mock clips，并合成 final work。
...
- 单组最终 clip/work 目标为 8-12 秒；扩展到 2-3 组时，有通向 10-15 秒最终作品的路径。
...
- Phase 1 单组 clip 为 8-12 秒。
- 如果计划视频长度需要三组，每组 4-5 秒，最终作品为 12-15 秒。

## 已确认产品决策

以下决策来自 2026-04-26 的人工产品 review，已作为进入 PLAN 的输入。

1. 核心分镜组数量由计划生成的视频长度自动判断：一组、两组或三组。
