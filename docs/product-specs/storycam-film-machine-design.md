...
4. canonical MVP 创作闭环必须保留：输入想法 -> 生成剧本、人物资产、场景资产 -> 用户确认 -> 生成 1 份 9 帧分镜脚本和 1 个核心分镜组 -> 可选点击中心主图扩展 8 帧 -> 为该核心分镜组生成一个 Seedance 2.0 视频片段 -> 用户确认片段 -> 系统建议拼接 -> 最终作品。
...
- 核心分镜代表图由对应组 `storyboard_script.frames[0].imagePrompt` 生成。
...
- 扩展卡来自已生成的结构化分镜脚本；每张卡的图片使用该帧原始 `imagePrompt`。
- 扩展卡默认不触发视频生成。
