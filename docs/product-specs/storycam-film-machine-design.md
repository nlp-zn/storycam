...
4. canonical MVP 创作闭环必须保留：输入想法 -> 生成剧本、人物资产、场景资产 -> 生成角色/场景资产图并由用户确认 story-world -> 基于剧本和资产生成 1 份 9 帧分镜脚本和 1 个核心分镜组 -> 基于资产图和第 1 帧脚本生成核心分镜图 -> 可选点击中心主图扩展 8 帧 -> 基于 9 帧脚本和已生成分镜图组装 Seedance 2.0 clip prompt packet 并生成一个视频片段 -> 在片段生成页内预览、重拍、生成最终作品并导出 MP4。
